...
class MatchResult(Base):
    ...
    grn_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("goods_receipt_notes.id"), nullable=True)
    ...
    purchase_order: Mapped[Optional["PurchaseOrder"]] = relationship(back_populates="match_results")
    grn: Mapped[Optional["GoodsReceiptNote"]] = relationship(back_populates="match_results")
...
class MatchLineResult(Base):
    ...
    grn_line_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("goods_receipt_lines.id"), nullable=True)

    status: Mapped[str] = mapped_column(String(20), nullable=False)

    po_unit_price: Mapped[Optional[Decimal]] = mapped_column(Numeric(12, 4), nullable=True)
    invoice_unit_price: Mapped[Optional[Decimal]] = mapped_column(Numeric(12, 4), nullable=True)
    ...
    po_quantity: Mapped[Optional[Decimal]] = mapped_column(Numeric(12, 4), nullable=True)
    grn_quantity: Mapped[Optional[Decimal]] = mapped_column(Numeric(12, 4), nullable=True)
    invoice_quantity: Mapped[Optional[Decimal]] = mapped_column(Numeric(12, 4), nullable=True)
    ...
    po_line: Mapped[Optional["PurchaseOrderLine"]] = relationship(back_populates="match_line_results")
    grn_line: Mapped[Optional["GoodsReceiptLine"]] = relationship(back_populates="match_line_results")
