...
def _get_encryption_key() -> bytes:
    """Decode the base64 field encryption key once and cache it."""
...
def create_refresh_token(data: dict) -> str:
    ...
    jti = secrets.token_hex(16)  # unique token ID for invalidation
...
# Used for: IMAP passwords, OAuth refresh tokens, ERP API keys stored in DB.
...
_IP_SALT = os.getenv("IP_HASH_SALT", "backbone-ip-salt")


def hash_ip(ip: str) -> str:
    """One-way hash of IP for audit logs — identifiable to us, pseudonymised to others."""
...
def generate_api_key() -> tuple[str, str]:
    """Generate (raw_key, hashed_key). Store only the hash."""
