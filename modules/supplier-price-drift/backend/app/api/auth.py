...
- POST /auth/token    — login with email + password, returns JWT pair
- POST /auth/refresh  — exchange refresh token for new access token (silent)
...
async def login(request: Request, body: LoginRequest, db: AsyncSession = Depends(get_db)):
    ...
    # Lockout check (always check before verifying password to avoid timing leaks)
    ...
    if not user or not verify_password(body.password, user.hashed_password) or not user.is_active:
        ...
        # Identical error for both "no user" and "wrong password" — prevents account enumeration
...
async def refresh_token(body: RefreshRequest, db: AsyncSession = Depends(get_db)):
    ...
    # Single-use enforcement: reject if JTI doesn't match stored value
